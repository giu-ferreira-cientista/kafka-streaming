...
df_json.select(from_json(df_json.json, mySchema).alias('raw_data'))   .select('raw_data.*')   .filter("nome is not NULL")   .writeStream   .trigger(once=True)   .format("console")   .start() 
  #.awaitTermination()
...
schema_output = StructType([StructField('label', IntegerType()),                            StructField('score', DoubleType())])

df_json.select(from_json(df_json.json, mySchema).alias('raw_data'))   .select('raw_data.*')   .filter("nome is not NULL")   .filter("idade is not NULL")   .filter("pressao is not NULL")   .filter("peso is not NULL")   .filter("altura is not NULL")   .select('nome',           from_json(vader_udf(array('*')), schema_output).alias('response'))  .select('nome', 'response.*')   .writeStream   .trigger(once=True)   .format("console")   .start()   


# In[10]:


df_json.select(from_json(df_json.json, mySchema).alias('raw_data'))   .select('raw_data.*')   .filter("nome is not NULL")   .filter("idade is not NULL")   .filter("pressao is not NULL")   .filter("peso is not NULL")   .filter("altura is not NULL")   .select('nome',           from_json(vader_udf(array('*')), schema_output).alias('response'))  .select('nome', 'response.*')   .select(
